...
const ResourceDetail = ({ resourceId, onBack }) => {
  ...
  useEffect(() => {
    ...
    if (resourceId) fetchResourceDetails();
  }, [resourceId]);

  if (isLoadingDetail) {
    return <LoadingSpinner label="Lädt Ressourcendetails..." />;
  }
  ...
  if (errorDetail) {
    return (
      <>
        <ErrorMessage
          title="Fehler beim Laden der Ressourcendetails"
          message={`Fehler: ${errorDetail}`}
          hint="Bitte überprüfen Sie Ihre Netzwerkverbindung oder versuchen Sie es später erneut."
        />
    ...
    return (
      <>
        <ErrorMessage
          ...
          message="Die Ressource konnte nicht geladen werden."
          hint={`ID: ${resourceId}`}
          variant="info"
        />
        <BackButton onBack={onBack} label="Zurück zu allen Ressourcen" />
      ...
      <div className="border-t border-gray-200 pt-8 mt-8 text-gray-600 text-sm grid grid-cols-1 md:grid-cols-2 gap-4">
        {authorId && (
          <p className="flex items-center">
            <strong className="mr-2">Autor-ID:</strong>
